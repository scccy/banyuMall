...
3. **LR-005**: Feign客户端配置规范
4. **LR-006**: 微服务依赖排除规则
5. **LR-007**: 路由一致性规范
6. **DEV-007**: 网关接口同步规则
7. **DEV-008**: 文档管理规则
8. **DEV-012**: 统一响应数据类和实体类规范
9. **PERF-001**: Spring Boot启动性能优化规则
...
- **总规则数**: 21个
- **开发规范类**: 10个
- **架构设计类**: 6个（LR-001缓存配置原则、LR-002 MyBatis-Plus依赖版本一致性、LR-003模块设计与文档标准化、LR-005 Feign客户端配置、LR-006微服务依赖排除、LR-007路由一致性）
- **配置管理类**: 1个
- **依赖管理类**: 1个
- **性能优化类**: 1个
- **对象存储类**: 1个
