...
#### LR-002: 模块设计优先规则
- **触发**: 新建或重构任何模块时
- **核心**: 必须先查看设计文档，根据设计文档生成代码，使用Excel表格格式和跳转链接，按模块名称创建文件夹组织文档，生成开发文档和接口执行测试
- **文件**: `LR-002_MODULE_DESIGN_FIRST.md`

#### LR-003: 微服务依赖排除规则
- **触发**: 新微服务模块创建或依赖配置修改
- **核心**: 排除Logback和Jackson、统一使用Log4j2和FastJSON2
- **文件**: `LR-003_MICROSERVICE_DEPENDENCY_EXCLUSION.md`
...
3. **DEV-007**: 网关接口同步规则
4. **DEV-008**: 文档管理规则
5. **DEV-012**: 统一响应数据类和实体类规范
6. **PERF-001**: Spring Boot启动性能优化规则
...
### 🚀 **新模块开发**
1. **LR-002**: 模块设计优先规则（按模块名称创建文件夹组织文档）
2. **DEV-007**: 网关接口同步规则
3. **LR-003**: 微服务依赖排除规则
4. **DEV-012**: 统一响应数据类和实体类规范
5. **DEV-001**: 基础开发规范
6. **DEV-005**: 错误处理规范

### 🔧 **代码开发**
1. **DEV-012**: 统一响应数据类和实体类规范
2. **DEV-001**: 基础开发规范
3. **DEV-002**: 代码提交规范
4. **DEV-003**: 测试规范
5. **DEV-005**: 错误处理规范
...
- **总规则数**: 17个
- **开发规范类**: 9个
- **架构设计类**: 3个（LR-001缓存同步、LR-002模块设计优先、LR-003微服务依赖排除）
- **配置管理类**: 1个
- **依赖管理类**: 1个
- **性能优化类**: 1个
- **对象存储类**: 1个
...
- **设计模板**: `infra/moudleDocs/xxxxxx-design_templates.md`
- **API测试模板**: `infra/moudleDocs/xxxxxx-api-test-template.md`
- **开发文档模板**: `infra/moudleDocs/xxxxxx-development-template.md`
- **接口测试验证模板**: `infra/moudleDocs/xxxxxx-interface-test-validation-template.md`
