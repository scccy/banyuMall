...
public enum WechatWorkErrorCode {
    // 授权相关错误
    AUTH_FAILED(1001, "授权失败"),
    INVALID_CODE(1002, "无效的授权码"),
    USER_NOT_FOUND(1003, "用户不存在"),
    
    // API调用错误
    API_CALL_FAILED(2001, "API调用失败"),
    TOKEN_EXPIRED(2002, "访问令牌过期"),
    RATE_LIMIT_EXCEEDED(2003, "接口调用频率超限"),
    
    // 数据同步错误
    SYNC_FAILED(3001, "数据同步失败"),
...
public class WechatWorkGlobalExceptionHandler {
    ...
    @ExceptionHandler(WechatWorkException.class)
    public ResponseEntity<ApiResponse<Void>> handleWechatWorkException(WechatWorkException e) {
        return ResponseEntity.ok(ApiResponse.error(e.getCode(), e.getMessage()));
    }
    
    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiResponse<Void>> handleException(Exception e) {
        return ResponseEntity.ok(ApiResponse.error(5001, "系统内部错误"));
    }
}
```
