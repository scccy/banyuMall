package com.origin.banyu.publisher.util;
 ...
 * 作者: scccy
 * 创建时间: 2025-07-31
 * 已废弃：taskConfig现在是前端自定义字段，不再需要校验和解析
 */
@Slf4j
@Component
public class TaskConfigParser {
     ...
     * @return 是否有效
     */
    public boolean validateTaskConfig(Integer taskTypeId, String jsonConfig) {
        if (jsonConfig == null || jsonConfig.trim().isEmpty()) {
            return true; // 允许空配置
        }
        ...
        try {
            // 先解析JSON字符串为Map
            Map<String, Object> config = parseFromJson(jsonConfig);
            return validateTaskConfigInternal(taskTypeId, config);
        } catch (Exception e) {
     ...
     * @return 是否有效
     */
    private boolean validateTaskConfigInternal(Integer taskTypeId, Map<String, Object> config) {
        try {
            switch (taskTypeId) {
                case 1: // 点赞任务
